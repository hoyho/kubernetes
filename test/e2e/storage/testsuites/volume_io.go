...
func (t *volumeIOTestSuite) defineTests(driver TestDriver, pattern testpatterns.TestPattern) {
	...
	ginkgo.It("should write files of various sizes, verify size, validate content [Slow]", func() {
		...
		var driverSizeRange, testSuitSizeRange volume.SizeRange
		...
		if len(t.tsInfo.supportedSizeRange.Max) == 0 {
			testSuitSizeRange.Max = maxValidSize
		} else {
			testSuitSizeRange.Max = t.tsInfo.supportedSizeRange.Max
		}
		...
		testQuantity, err = resource.ParseQuantity(t.tsInfo.supportedSizeRange.Max)
		if err != nil {
			framework.ExpectNoError(err)
		}
